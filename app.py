...
# 1. 초기 설정 (버전 v5.0: Gems 연동 딥 리서치 프롬프트 생성 기능 탑재)
st.set_page_config(page_title="Wonju AI Quant Lab v5.0", layout="wide", page_icon="💎")
...
def get_available_ai_models():
    try:
        ...
        # 리스트에 없는 기타 모델들 추가 (혹시 모를 신규 모델 대응)
...
if df is not None:
    ...
    st.title(f"📈 {target_ticker} Pro Dashboard v5.0")
    ...
    # [핵심] Gems 전용 딥 리서치 프롬프트 생성 (복사 붙여넣기용)
    st.subheader("🚀 Deep Research 연동 (Gems)")
    with st.expander("🔍 Gems 심층 분석용 데이터 팩 추출 (클릭하여 열기)", expanded=False):
        st.write("아래 내용을 복사하여 Gems에 붙여넣으세요. 대시보드의 실시간 데이터와 Gems의 검색 능력이 결합됩니다.")
